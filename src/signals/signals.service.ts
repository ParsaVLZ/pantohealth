...
export class SignalsService {
  ...
  async processAndSaveXRayData(xrayData: Record<string, any>){
    for (const [deviceId, payload] of Object.entries(xrayData)) {
      if (!payload || !payload.data) { 
        this.logger.error(`Invalid data received for device ${deviceId}: ${JSON.stringify(payload)}`);
        continue; 
      }
  ...
  async createSignal(dto: CreateXRayDto){
    return this.xRayModel.create(dto);
  }

  async updateSignal(id: string, updateDto: Partial<CreateXRayDto>){
